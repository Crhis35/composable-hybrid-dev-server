...
const options = {
  ...
  router: (req) => {
    if (MRT_RULES.length) {
      ...
      if (match) {
        console.log(`Proxyinga ${req.path} to ${PWA_ORIGIN}...`);
        return PWA_ORIGIN;
      }
    } else {
    ...
    console.log(`Proxyingb ${req.path} to ${SFCC_ORIGIN}...`);
    return SFCC_ORIGIN;
  },
...
// Start the server
const port = 8001; // Choose the port you want to use for the proxy server
server.listen(port, () => {
  console.log(`Proxy server listening on port ${port} (HTTPS)`);
}); */
